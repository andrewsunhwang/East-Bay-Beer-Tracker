...
| `CLAUDE_MODEL` | `claude-sonnet-5` | Parsing model |
...
3. The text is content-hashed. If it's identical to the last successful scrape of that URL, the previously-extracted beer list is reused and **no Claude call is made**. Only changed pages go to the LLM.
4. Claude extracts a structured list of beers (name, style, ABV, description, availability, style family) via the SDK's `messages.parse` with a Pydantic schema; the result is cached against the text hash.
5. Beers are matched to existing rows by normalized name: existing beers are updated, brand-new beers are inserted (and trigger alert emails), and beers missing from the page are marked not current.
6. Every run is recorded in the scrape log visible in the admin panel.
